...
def vec_sum(veclist, D): 
    ...
    if len(veclist) == 0:
        return Vec(D, {})
    v = vec_sum(veclist[1:], D)
    for key, value in veclist[0].f.items():
        if key in v.f:
            v.f[key] += value
        else:
            v.f[key] = value
...
def scale_vecs(vecdict):
    ...
    True
    '''
    return [Vec(v.D, {d:value/k for d, value in v.f.items()}) for k,v in vecdict.items()]
...
def GF2_span(D, L):
    ...
    for i in range(2**len(L)):
        result.append(vec_sum([L[j] for j in range(len(L)) if 1<<j & i], D))
    return result
