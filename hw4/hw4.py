...
indep_vec_1 = Vec({1,2,3,4}, {1,0,0,0})
indep_vec_2 = Vec({1,2,3,4}, {0,1,0,0})
indep_vec_3 = Vec({1,2,3,4}, {0,0,1,0})
indep_vec_4 = Vec({1,2,3,4}, {0,0,0,1})
...
def superset_basis(S, L):
    ...
    result = S[:]
    for b in L:
        if not does_solution_exist(coldict2mat(result), b):
            result.append(b)
    return result

def does_solution_exist(A,b):
    u = solve(A,b)
    res = A*u-b
    return res*res < 10**-14
...
def exchange(S, A, z):
    ...
    for w in W:
        tmp = S[:]
        tmp.remove(w)
        if does_solution_exist(coldict2mat(tmp), w):
            return w
    return z
