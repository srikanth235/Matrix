...
def move2board(v): 
    ...
    '''
    return Vec({'y1','y2','y3'}, {k:c/v.f['y3'] for k,c in v.f.items()})
...
## Task 3
domain = {(a, b) for a in {'y1', 'y2', 'y3'} for b in {'x1', 'x2', 'x3'}}
L = rowdict2mat(make_equations(358,36,0,0) + make_equations(329,597, 0,1)+ make_equations(592,157,1,0)+make_equations(580, 483, 1,1)+[Vec(domain,{('y1','x1'):1})])
...
def mat_move2board(Y):
    ...
    '''
    return coldict2mat([move2board(c) for c in mat2coldict(Y).values()])
